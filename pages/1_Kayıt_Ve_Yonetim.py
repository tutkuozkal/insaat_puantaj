...
# st.title("⚙️ Sistem Tanımlamaları ve Akıllı Veri Aktarımı")

# # Sekmeleri Oluştur (Şirket Havuzu eklendi, Alan ve Görev eski fonksiyonelliğinde korundu)
...
# # --- 1. AKILLI TOPLU EKLE (Şirket ve Görev Havuzunu Otomatik Doldurur) ---
# with tab1:
#     col_a, col_b = st.columns([1, 1])
#     with col_a:
#         st.subheader("📥 Tek Excel ile Sistemi Kur")
#         # Şirket sütunu eklenmiş revize şablon
#         template_data = {
#             "ad_soyad": ["Ahmet Yılmaz", "Mehmet Demir"], 
#             "tc_no": ["10000000010", "10000000012"], 
#             "telefon": ["05321112233", "05334445566"],
#             "gorev": ["Kalıpçı", "Demirci"], 
#             "sirket": ["Alfa İnşaat", "Beta Yapı"], 
#             "kan_grubu": ["A+", "B-"], 
#             "dogum_tarihi": ["1985-01-01", "1990-05-20"]
#         }
#         buffer = io.BytesIO()
#         with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
#             pd.DataFrame(template_data).to_excel(writer, index=False)
#         st.download_button("📄 Yeni Excel Şablonunu İndir", buffer.getvalue(), "akilli_personel_sablonu.xlsx")
        
#         uploaded_file = st.file_uploader("Doldurulan Dosyayı Yükleyin", type=['xlsx'])
#         if uploaded_file and st.button("🚀 Her Şeyi Otomatik Aktar"):
...
#                 try:
#                     # A. Görevi havuzda yoksa otomatik ekle
#                     cur.execute("INSERT INTO gorev_havuzu (gorev_adi) VALUES (%s) ON CONFLICT DO NOTHING", (str(row['gorev']).strip(),))
                    
#                     # B. Şirketi havuzda yoksa otomatik ekle ve ID al
#                     cur.execute("INSERT INTO sirketler (sirket_adi) VALUES (%s) ON CONFLICT (sirket_adi) DO NOTHING", (str(row['sirket']).strip(),))
#                     cur.execute("SELECT id FROM sirketler WHERE sirket_adi = %s", (str(row['sirket']).strip(),))
#                     s_id = cur.fetchone()[0]
                    
#                     # C. Personeli ekle (unique_ad_soyad veya tc_no çakışırsa eklemez)
#                     cur.execute("""
...
#                 except Exception:
#                     skipped += 1
#                     conn.rollback() # Hatalı satırda işlemi geri al, döngüye devam et
#                     continue
            
#             conn.commit(); conn.close()
#             st.success(f"✅ {success} Personel ve bağlı havuzlar kuruldu. (Atlanan: {skipped})")
#             time.sleep(1); st.rerun()

#     with col_b:
#         st.subheader("👤 Tekil Personel Kaydı")
#         # Eski manuel kayıt formu (Şirket seçimi dahil)
#         with st.form("tekil_kayit_form", clear_on_submit=True):
#             f_ad = st.text_input("Ad Soyad*")
#             s_df = query_to_df("SELECT sirket_adi FROM sirketler ORDER BY sirket_adi")
#             f_sirket = st.selectbox("Şirket", options=s_df["sirket_adi"].tolist() if not s_df.empty else ["Genel"])
#             g_df = query_to_df("SELECT gorev_adi FROM gorev_havuzu ORDER BY gorev_adi")
#             f_gorev = st.selectbox("Branş", options=g_df["gorev_adi"].tolist() if not g_df.empty else ["Genel"])
#             f_tc = st.text_input("TC No")
#             if st.form_submit_button("Sisteme Kaydet"):
#                 # Manuel kayıt mantığı...
#                 pass
...
#     with col_a:
#         with st.form("sirket_ekle_form"):
#             s_adi = st.text_input("Şirket Adı")
#             if st.form_submit_button("Şirketi Ekle"):
#                 conn = get_connection(); cur = conn.cursor()
#                 cur.execute("INSERT INTO sirketler (sirket_adi) VALUES (%s) ON CONFLICT DO NOTHING", (s_adi.strip(),))
#                 conn.commit(); conn.close(); st.rerun()
#     with col_b:
#         s_list = query_to_df("SELECT id, sirket_adi FROM sirketler ORDER BY sirket_adi")
#         for _, r in s_list.iterrows():
#             c1, c2 = st.columns([4,1]); c1.info(f"🏢 {r['sirket_adi']}")
...
# # --- 3. ALAN HAVUZU (Manuel Giriş İçin Korundu) ---
# with tab3:
#     col_a, col_b = st.columns([1, 2])
#     with col_a:
#         with st.form("alan_ekle_form"):
#             a_adi = st.text_input("Alan/Blok Adı")
#             if st.form_submit_button("Alanı Kaydet"):
#                 conn = get_connection(); cur = conn.cursor()
#                 cur.execute("INSERT INTO alanlar (alan_adi) VALUES (%s)", (a_adi.strip(),))
#                 conn.commit(); conn.close(); st.rerun()
#     with col_b:
#         a_df = query_to_df("SELECT id, alan_adi FROM alanlar ORDER BY alan_adi")
#         for _, r in a_df.iterrows():
#             c1, c2 = st.columns([4,1]); c1.write(f"🏗️ {r['alan_adi']}")
...
# # --- 4. GÖREV HAVUZU (Eski Hali Korundu) ---
# with tab4:
#     col_a, col_b = st.columns([1, 2])
#     with col_a:
#         with st.form("gorev_ekle_form"):
#             g_adi = st.text_input("Branş/İş Tanımı")
#             if st.form_submit_button("Görevi Kaydet"):
#                 conn = get_connection(); cur = conn.cursor()
#                 cur.execute("INSERT INTO gorev_havuzu (gorev_adi) VALUES (%s) ON CONFLICT DO NOTHING", (g_adi.strip(),))
#                 conn.commit(); conn.close(); st.rerun()
#     with col_b:
#         g_df = query_to_df("SELECT id, gorev_adi FROM gorev_havuzu ORDER BY gorev_adi")
#         for _, r in g_df.iterrows():
#             c1, c2 = st.columns([4,1]); c1.write(f"🛠️ {r['gorev_adi']}")
...
# # --- 5. PERSONEL DETAY & DOSYA (Unique Ad Soyad Uyumlu) ---
# with tab5:
...
#     if not p_df.empty:
#         # İsim soyisimler unique olduğu için doğrudan isimle seçtiriyoruz
#         secilen_ad = st.selectbox("Düzenlenecek Personel Seçin:", p_df["ad_soyad"].tolist())
#         p_row = p_df[p_df["ad_soyad"] == secilen_ad].iloc[0]
#         p_id = int(p_row['id'])

#         with st.form(key=f"detay_form_{p_id}"):
#             c1, c2, c3 = st.columns([1, 2, 2])
#             with c2:
#                 u_ad = st.text_input("Ad Soyad", value=str(p_row['ad_soyad']))
#                 # Şirket Eşleşmesi
...
#                 p_gorev = str(p_row['gorev']).strip()
#                 g_idx = g_opts.index(p_gorev) if p_gorev in g_opts else 0
#                 u_brans = st.selectbox("Branş", g_opts, index=g_idx)
...
#                 sid = cur.fetchone()[0]
#                 cur.execute("UPDATE personeller SET ad_soyad=%s, gorev=%s, tc_no=%s, sirket_id=%s WHERE id=%s", (u_ad, u_brans, u_tc, sid, p_id))
#                 conn.commit(); conn.close(); st.success("Güncellendi!"); time.sleep(1); st.rerun()

# # --- 6. İSTATİSTİK ---
# with tab6:
#     st.subheader("📊 Personel Envanteri")
#     res_df = query_to_df("SELECT p.ad_soyad, s.sirket_adi, p.gorev FROM personeller p LEFT JOIN sirketler s ON p.sirket_id = s.id")
#     if not res_df.empty:
#         st.dataframe(res_df, use_container_width=True)
#         st.bar_chart(res_df['sirket_adi'].value_counts())
